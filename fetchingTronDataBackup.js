...
const db = new sqlite3.Database('tronData.db', sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE, async (err) => {
    if (err) {
...
async function listenToBlocks() {
    try {
        ...
        if (blockNum !== 54075718 && blockNum == 0 || blockNum == null || blockNum == 1) {
                    ...
                    try {
                        ...
                        for (const txs of block.transactions) {
                            ...
                            const transactionData = {
                                ...
                                blockHash: blockData.blockHash,
                                blockNumber: blockData.blockNumber,
                                fromAddress: txs.raw_data.contract[0].parameter.value.owner_address,
                                gasPrice: txs.raw_data.contract[0].parameter.value.call_value,
                                result: txs.ret[0].contractRet,
                                input: txs.raw_data.contract[0].parameter.value.data,
                                stakedAssetReleasedBalance: txs.raw_data.contract[0].parameter.value.balance,
                                resource: txs.raw_data.contract[0].parameter.value.resource,
                                timestamp: convertUnixTimestamp(txs.raw_data.timestamp),
                                expiration: convertUnixTimestamp(txs.raw_data.expiration),
                                ...
                                type: txs.raw_data.contract[0].type,
                                ownerAddress: txs.raw_data.contract[0].parameter.value.owner_address,
                                contractAddress: txs.raw_data.contract[0].parameter.value.contract_address,
                                resourcesTakenFromAddress: txs.raw_data.contract[0].parameter.value.receiver_address,
                                contractData: txs.raw_data.contract[0].parameter.value.data,
                            };
                            ...
                            db.run(txInsertSql, [
                                ...
                                tronWeb.address.fromHex(transactionData.resourcesTakenFromAddress),
